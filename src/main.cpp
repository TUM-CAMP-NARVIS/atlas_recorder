...
int main(int argc, char **argv)
{
    ...
    cmd_parser.RegisterOption("-l|--max-block-length",
                              "Limit the the file block length to N frames (default: 9000)",
                              1,
                              [&](const std::vector<char *> &args) {
    ...
    {
        char *s = argv[argc -1];
        base_filename = std::string(s);
        if (std::count(base_filename.begin(), base_filename.end(), '.') != 1)
            std::cout << "filename must contain one '.' and a file extension" << std::endl;
    }
    else
    {
        std::cout << "atlas_recorder [options] output.mkv" << std::endl << std::endl;
